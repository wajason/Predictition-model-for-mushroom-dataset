...
### Prerequisites
- **R**: Install R (version 4.0 or higher) from [CRAN](https://cran.r-project.org/).
- **Python**: Install Python (version 3.8 or higher) from [Python.org](https://www.python.org/downloads/) or via Anaconda.
- **Quarto**: Install Quarto from [Quarto.org](https://quarto.org/docs/get-started/) to render the `.qmd` file.
...
2. **Install R Dependencies**:
   Open R and run the following to install required packages:
   ```R
...
4. **Set Up Python Path in R**:
   Ensure `reticulate` in R points to the correct Python environment. Edit the `.qmd` file if needed:
   ```R
   library(reticulate)
   use_python("path/to/your/python", required = TRUE)  # Update the path as needed
   ```

5. **Install Fonts for PDF Rendering**:
   The PDF uses the `Microsoft JhengHei` font for Chinese characters. Ensure this font is installed on your system, or modify the `include-in-header` section of the `.qmd` file to use a different font available on your system.
...
- **Quarto Markdown Version** (`Predictition_model_for_mushroom_dataset_on_github.qmd`):
  ...
  - To render the `.qmd` file into a PDF or HTML:
    ...
    2. Run the following command in your terminal:
       ```bash
       quarto render Predictition_model_for_mushroom_dataset_on_github.qmd --to pdf
       ```
    3. The rendered output will be generated in the same directory.
...
- **Reproducing Results**:
  ...
  - Run the entire `.qmd` file using Quarto to reproduce the full analysis:
    ```bash
    quarto render Predictition_model_for_mushroom_dataset_on_github.qmd
    ```
...
- **Hyperparameter Optimization**:
  - The Optuna section is commented out in the `.qmd` file. Uncomment and run it to perform hyperparameter tuning for the models:
    ```python
    import optuna
...
- **Jason** (Project Lead): [your-email@example.com](mailto:your-email@example.com)
